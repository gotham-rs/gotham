use gotham::router::route::dispatch::{finalize_pipeline_set, new_pipeline_set, PipelineSet};
...
type PipelineHandle =
    borrow_bag::Handle<NewPipeline, <() as borrow_bag::Append<NewPipeline>>::Navigator>;
...
pub fn make_pipelines() -> ((PipelineHandle, ()), PipelineSet<(NewPipeline, ())>) {
    ...
    let (pipelines, global) = pipeline_set.add(new_pipeline().add(redis_middleware).build());
