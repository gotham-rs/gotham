...
/// This implementation allows you to undertake any Middleware logic that is necessary for your
/// applications needs.
///
...
impl Middleware for ExampleMiddleware {
    ...
    {
        // Prior to letting Request handling proceed we create some new data and add it to `state`.
        state.put(ExampleMiddlewareData {
            stock_remaining: 100,
        });

        // We're finished doing processing so allow other components to continue processing the
        // request.
...
///
/// This handler expects that the `ExampleMiddleware` has already been executed by Gotham before
/// the handler is invoked. As a result of that middleware being run our handler knows that it must
...
fn router() -> Router {
    ...
    // ordering which is established by calls to the `add` method.
    //
    // A pipeline is considered complete once the build method is called and can no longer
    // be modified.
    //
    // Applications may have many Pipelines or even Pipelines containing Pipelines, however
    // many applications will get sufficent power and flexibility from a `single_pipeline` as
    // shown here.
    //
    // Future examples will go into the flexibility of Pipelines in much more detail.
    ...
    // Notice we've derived from build_simple_router which has been present in all our examples up
    // until this point. Under the hood it has simply been creating an empty set of Pipelines on
    // your behalf.
    //
