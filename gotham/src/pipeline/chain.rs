...
pub trait PipelineHandleChain<P, B>: RefUnwindSafe {
    ...
    fn call<F>(&self, pipelines: &PipelineSet<P>, state: State, f: F) -> Box<HandlerFuture<B>>
    where
...
impl<'a, P, T, N, U, B> PipelineHandleChain<P, B> for (Handle<Pipeline<T, B>, N>, U)
where
    T: NewMiddlewareChain<B>,
    T::Instance: Send + 'static,
    U: PipelineHandleChain<P, B>,
    P: Lookup<Pipeline<T, B>, N>,
    N: RefUnwindSafe,
{
    fn call<F>(&self, pipelines: &PipelineSet<P>, state: State, f: F) -> Box<HandlerFuture<B>>
    where
...
impl<P, B> PipelineHandleChain<P, B> for () {
    fn call<F>(&self, _: &PipelineSet<P>, state: State, f: F) -> Box<HandlerFuture<B>>
    where
        F: FnOnce(State) -> Box<HandlerFuture<B>> + Send + 'static,
    {
